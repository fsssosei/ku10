window.__utils = {
  ...
  checkCurrgame(input = '', isZJ) {
    if (isZJ) {
      const startWithBlack = String(input).startsWith('0');
      const grouped = window.__utils.groupArr(input, 3);
      const typeArr = [];
      for (let i of grouped) {
        typeArr.push(i[0]);
      }

      const colorArrStr = typeArr.join('');
      const hasBlackCoherence = colorArrStr.indexOf('00');
      const hasWhiteCoherence = colorArrStr.indexOf('11');
      const hasCoherence = (hasBlackCoherence !== -1) || (hasWhiteCoherence !== -1);
      return startWithBlack && (!hasCoherence);
    } else {
      return true;
    }
  }
